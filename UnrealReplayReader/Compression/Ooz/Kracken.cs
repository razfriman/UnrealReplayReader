...
public unsafe class Kracken
{
    public byte[] Decompress(byte[] compressedInput, int uncompressedSize)
    {
        ...
        fixed (byte* scratchPtr = decoder.Scratch)
        {
            ...
            while (remainingBytes != 0)
            {
                ...
                {
                    throw new ReplayException($"Failed DecodeStep method");
                }
        ...
        int remainingDestinationBytes, byte* source, int sourceBytesleft, byte* scratch)
    {
        ...
        {
            throw new ReplayException($"Decoder type {decoder.Header.DecoderType} not supported");
        }

        if (source > sourceEnd)
        {
            throw new ReplayException($"Index out of range of source array");
        }
        ...
        if (decoder.Header.UseChecksums)
        {
            ...
            {
                throw new ReplayException($"Invalid checksum. Found {checksum} need {quantumHeader.Checksum}");
            }
        }
        ...
        switch (decoder.Header.DecoderType)
        {
            case DecoderTypes.Mermaid:
                ...
                break;
            default:
                throw new ReplayException($"Decoder type {decoder.Header.DecoderType} currently not supported");
        }
    ...
    private KrackenHeader ParseHeader(byte* source)
    {
        ...
        if ((firstByte & 0xF) == 0xC)
        {
            if (((firstByte >> 4) & 3) != 0)
            {
                throw new ReplayException($"Failed to decode header. ((source[0] >> 4) & 3) != 0");
            }
        ...
        throw new ReplayException($"Failed to decode header. (source[0] & 0xF) != 0xC");
    }
    ...
    private KrakenQuantumHeader ParseQuantumHeader(byte* source, bool useChecksums, out int bytesRead)
    {
        ...
        throw new ReplayException($"Failed to parse KrakenQuantumHeader");
    }
        ...
        bool forceMemmove, byte* scratch, byte* scratchEnd)
    {
        ...
        {
            throw new ReplayException($"DecodeBytes: Too few bytes ({sourceEnd - source}) remaining");
        }
        ...
        if (chunkType == 0)
        {
            ...
            else
            {
                ...
                {
                    throw new ReplayException($"Reserved bits must not be set");
                }
        ...
        while (destinationEnd - destination != 0)
        {
            ...
            else
            {
                ...
                if (sourceUsed < destinationCount)
                {
                    ...
                    {
                        throw new ReplayException($"Failed to run MermaidReadLzTable");
                    }
                    ...
                    if (!MermaidProcessLzRuns(mode,
                            ...
                            (MermaidLzTable*)temp))
                    {
                        throw new ReplayException($"Failed to run MermaidProcessLzRuns");
                    }
                }
        ...
        long offset, byte* scratch, byte* scratchEnd, MermaidLzTable* lz)
    {
        ...
        else
        {
            if (sourceEnd - source < 2)
            {
                throw new ReplayException($"MermaidReadLzTable: Too few bytes ({sourceEnd - source}) remaining");
            }
        ...
        if (off16Count == 0xFFFF)
        {
            ...
            {
                throw new ReplayException($"MermaidReadLzTable: scratch + offset16LowCount * 2 > scratchEnd");
            }
        ...
        if (temp != 0)
        {
            ...
            {
                throw new ReplayException($"MermaidReadLzTable: Not enough remaining scratch space");
            }
        ...
        int destinationSize, long offset, byte* destinationEnd, MermaidLzTable* lz)
    {
        ...
        {
            throw new ReplayException($"MermaidProcessLzRuns: Failed to read decompress source bytes");
        }
        ...
        byte* destinationStart, byte* sourceEnd, MermaidLzTable* lz, int* savedDist, int startOff)
    {
        ...
        while (cmdStream < cmdStreamEnd)
        {
            ...
            else if (flag > 2)
            {
                ...
                {
                    throw new ReplayException($"MermaidMode1: off32Stream == off32StreamEnd");
                }
                ...
                {
                    throw new ReplayException($"MermaidMode1: destinationEnd - destination < length");
                }
            ...
            else if (flag == 1)
            {
                ...
                {
                    throw new ReplayException($"MermaidMode1: off16Stream == off16StreamEnd");
                }
            ...
            else
            {
                if (sourceEnd - lengthStream == 0)
                {
                    throw new ReplayException($"MermaidMode1: sourceEnd - lengthStream == 0");
                }
    ...
    private int MermaidDecodeFarOffsets(byte* source, byte* sourceEnd, uint* output, uint outputSize, long offset)
    {
        ...
        if (offset < (0xC00000 - 1))
        {
            for (i = 0; i != outputSize; i++)
            {
                ...
                if (off > offset)
                {
                    throw new ReplayException($"MermaidDecodeFarOffsets: off ({off}) > offset ({offset})");
                }
            }
        ...
        for (i = 0; i != outputSize; i++)
        {
            ...
            if (off >= 0xc00000)
            {
                if (sourceCurrent == sourceEnd)
                {
                    throw new ReplayException($"MermaidDecodeFarOffsets: No remaining bytes");
                }
